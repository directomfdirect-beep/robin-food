                ...
                return (
                  <div key={s.key} className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                      isCurrent ? 'bg-black text-white' : isCompleted ? 'bg-acid/20 text-black' : 'bg-gray-100 text-gray-300'
                    }`}>
                      <Icon size={18} />
                    </div>
                    <div className="flex-1">
                      <p className={`font-bold text-sm ${isCurrent ? 'text-black' : isCompleted ? 'text-black' : 'text-gray-300'}`}>
                        {s.label}
                      </p>
                    </div>
                    {isCurrent && (
                      <span className="text-[10px] font-bold text-black bg-gray-100 px-2 py-1 rounded-lg">
                        Текущий
                      </span>
                    )}
                  </div>
          ...
          <div className="flex flex-col h-full">
              ...
              {messages.map((msg) => (
                ...
                >
                  <div className={`max-w-[80%] px-4 py-2.5 rounded-2xl ${
                    msg.sender === 'buyer' ? 'bg-black text-white' : msg.sender === 'system' ? 'bg-gray-100 text-gray-500' : 'bg-gray-100'
                  }`}>
            ...
            <div className="p-4 border-t border-gray-100 flex gap-2 flex-shrink-0">
              <input
                ...
                className="flex-1 px-4 py-3 bg-gray-100 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-acid/30"
              />
              <button
                onClick={handleSendMessage}
                disabled={!chatMessage.trim()}
                className="p-3 bg-black text-white rounded-2xl disabled:opacity-30 transition-colors"
              >
                <Send size={18} />
              </button>
            </div>
